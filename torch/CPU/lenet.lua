...
testset = {
   size = 10000,
   data = dataw.data[{{1,10000}}]:float(),
   label = dataw.label[{{1,10000}}]:float()
}
...
model = nn.Sequential()
-- cbfp stack - 1
model:add(nn.SpatialConvolution(1, 20, 5, 5, 1, 1, 0):init('weight', nninit.xavier))
model:add(nn.ReLU())
model:add(nn.SpatialMaxPooling(2, 2, 2, 2))
-- cbfp stack - 2
model:add(nn.SpatialConvolution(20, 50, 5, 5, 1, 1, 0):init('weight', nninit.xavier))
model:add(nn.ReLU())
model:add(nn.SpatialMaxPooling(2, 2, 2, 2))
-- hidden layer
...
function eval(dataset, batch_size)
   ...
   for i = 1,dataset.size,batch_size do
      ...
      inputs = torch.FloatTensor(size, 1, 28, 28)
