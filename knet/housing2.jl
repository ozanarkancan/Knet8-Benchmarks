...
Repository."
    ...
    @show atype = eval(parse(o[:atype]))
    w = Any[convert(atype, 0.1*randn(1,13)), zero(eltype(atype))]
    (xtrn,ytrn,xtst,ytst) = loaddata(o[:test]) # map(x->convert(atype,x), loaddata(o[:test]))
    (xtrn2,ytrn2,xtst2,ytst2) = map(KnetArray, (xtrn,ytrn,xtst,ytst))
    println((:epoch,0,:trn,loss(w,xtrn2,ytrn2),:tst,loss(w,xtst2,ytst2)))
    if o[:fast]
        @time train(w, xtrn, ytrn; lr=o[:lr], epochs=o[:epochs])
        println((:epoch,o[:epochs],:trn,loss(w,xtrn2,ytrn2),:tst,loss(w,xtst2,ytst2)))
    else
...
function train(w, x, y; lr=.1, epochs=20)
    for epoch=1:epochs
        xx = convert(KnetArray,x)
        yy = convert(KnetArray,y)
