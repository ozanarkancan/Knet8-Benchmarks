...
function main(args=ARGS)
    ...
    w = weights(o[:hidden]...; atype=KnetArray{Float32}, winit=o[:winit])
    dtrn = minibatch(xtrn, ytrn, o[:batchsize]; atype=Array{Float32})
    dtst = minibatch(xtst, ytst, o[:batchsize]; atype=Array{Float32})
...
function accuracy(w, dtst)
    ...
    for (x, ygold) in dtst
        xx = KnetArray(x)
        yygold = KnetArray(ygold)
        ypred = predict(w, xx)
        ncorrect += sum(yygold .* (ypred .== maximum(ypred,1)))
